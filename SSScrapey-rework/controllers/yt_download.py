...
def bigBoyChannelDownloader(scrapped_channels_with_todos,*, isDebug):
    ...
    chn_limit =          int(env_varz.YTDL_NUM_CHANNELS)
    vid_download_limit = int(env_varz.YTDL_VIDS_PER_CHANNEL)

    for channel in scrapped_channels_with_todos:
        print("    (bigboy) Will do these channels: " + channel.get('url'))

    metadata_Ytdl_list: List[Metadata_Ytdl] = []
    chn_counter = 0
    for channel in scrapped_channels_with_todos:
        # TODO 
        # #updated SQL database as 'processing'
...
def uploadAudioToS3(yt_meta: Metadata_Ytdl):
    ...
    vod_filename = os.path.basename(yt_meta.outFile)
    vod_id = yt_meta.link.replace("/videos/", "").replace("/", "")
    caption_keybase = env_varz.S3_CAPTIONS_KEYBASE + yt_meta.channel + "/" + vod_id # caption_keybase = channels/vod-audio/lolgeranimo/1747933567 
    ...
    try:
        ...
        s3.upload_file(os.path.abspath(yt_meta.outFile[5:]), 
                       env_varz.BUCKET_NAME, 
                       s3fileKey, 
                       ExtraArgs={ 'ContentType': 'audio/mpeg'}
                    )
        print ("    UPLOADING META !!!!!!!!!!!!!!!!! ")
        s3.put_object(Body=json.dumps(yt_meta.__dict__, default=lambda o: o.__dict__), 
...
def addTodoListS3(scrapped_channels:List[ScrappedChannel]):
